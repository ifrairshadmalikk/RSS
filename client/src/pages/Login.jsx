...
function SlideCaption({ active }) {
  return (
    <div className="absolute inset-x-4 bottom-4 text-white sm:inset-x-8 sm:bottom-8 lg:inset-x-12 lg:bottom-12">
      <p className="text-sm font-medium drop-shadow-sm sm:text-base lg:text-lg">{SLIDES[active].tagline}</p>
...
//
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white text-slate-900 lg:grid lg:h-screen lg:grid-cols-2 lg:overflow-hidden">
      {/* left: brand + form */}
      <div className="flex flex-1 flex-col overflow-y-auto bg-white px-5 py-6 sm:px-10 sm:py-8 lg:px-16 lg:py-8">
        ...
        {/* mobile slide banner */}
        <div className="relative mt-5 h-36 w-full overflow-hidden rounded-2xl sm:h-44 lg:hidden">
          <SlidePhotos active={slide} />
          <SlideCaption active={slide} />
        </div>

        {/* form */}
        <form onSubmit={handleSubmit} className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center py-4 lg:py-6">
          <p className="text-sm text-slate-400">{isSignup ? 'Start your journey' : 'Welcome back'}</p>
          <h1 className="mb-8 text-2xl font-bold leading-tight text-slate-900 sm:text-[26px]">
